...
public class MST extends CanvasController {// bai toan thuc hien theo thuat toan tham lam kruskal
	...
	public MST() {
		// set up mac dinh ban dau cac nut co nut truoc do la chinh no
		for (NodeFX x : circles) {
			x.node.previous = x.node;
		}
		// duyet tat ca cac nut co trong do thi xem cha cua cac nut
		for (NodeFX x : circles) {
		...
		for (Edge e : mstEdges) {
			...
			if (findParent(e.source.previous) != findParent(e.target.previous)) {
				...
				final String sa = "\t---->Unioned\n" + "\t\t->Node :" + e.source.name.trim() + "  Parent: "
						+ findParent(e.source.previous).name.trim() + "\n" + "\t\t->Node :" + e.target.name.trim()
						+ "  Parent: " + findParent(e.target.previous).name.trim() + "\n";
              // hien thi noi dung ben hiddenpane
               ...
               // cac duong sang mau xanh toi
				...
				// hieu ung chuyen tu mau nay sang may khac cac nut source
				...
				st.getChildren().add(ft);
				 //chuyen nut sang mau den roi chuyen sang mau cung mau voi viewer
